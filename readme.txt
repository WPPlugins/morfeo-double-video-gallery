=== MORFEO DOUBLE VIDEO GALLERY ===
Contributors: wpvideoplugins.com
Donate link: http://www.wpvideoplugins.com/morfeo-video-gallery/
Tags: youtube, flv, video, blogs, carousel, categories, category, content, CSS, featured, flash, gallery, home, image, jquery, page, picture, plugin, Post, posts, shortcode, sidebar, simple, site, slider, slideshow, Style, thumbnail, widget, wpmu
Requires at least: 2.0.2
Tested up to: 3.0.1
Stable tag: 4.3

With this plugin you can create video galleries with flv videos and/or youtube videos.

== Description ==

With this plugin you can create video galleries from flv videos or youtube videos. Widget. To display the plugin in your Posts type [morfeo_double ID /], where ID is the morfeo gallery ID.

For create and configure the video galleries press in your Wordpress admin: Morfeo double


Manual and config samples: http://www.wpvideoplugins.com/morfeo-basic-video/

Download full version morfeo video: http://www.wpvideoplugins.com/morfeo-video-gallery/

Download more video plugins: http://www.wpvideoplugins.com/
		

== Installation ==



1. Upload `morfeo-double` folder to the `/wp-content/plugins/` directory.
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Configure and create new video gallery in your Wordpress Admin: Morfeo double
4. Place [`morfeo_double` ID /] in your Posts, where ID is the gallery ID.

5. Admin params:

	- Video URL. Write the url of your videos. You can write the url of the flv videos, the url of the videos on youtube or the ID of the videos from youtube. The first video would be the first line, the second video the second line …
	
	- Video Titles. Write the titles of your videos. The first video would be the first line, the second video the second line …

	- Gallery Width. Number indicating the width of the carousel. You can use a number in pixels or percentage. For example: 100% or 400px.
	- Gallery Height. Number indicating the height of the carousel. You must use a number with px. For example 400px.


== Frequently Asked Questions ==
= What parameters should I set up to look good to me the morfeo? =
Once installed and activated, and the default plugin works correctly. The parameters are common to change the url videos. After adjusting the parameter width, height and get to the gallery is good.

== Screenshots ==

1. Sample.
2. Configuration.

== Changelog ==

= 2.1 =

* **NEW VERSION**

= 1.3 =

* **Pause button:** Pause button disappears, error fixed.

= 1.2 =

* **Youtube:** Optimization of the quality of the videos from youtube.
* **Search engines:** Code optimization for search engines.

= 1.0 =
