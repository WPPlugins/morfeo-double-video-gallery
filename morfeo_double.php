...
function morfeo_double_head() {
	
	$site_url = get_option( 'siteurl' );
			
			
}
function morfeo_double($content){
	$content = preg_replace_callback("/\[morfeo_double ([^]]*)\/\]/i", "morfeo_double_render", $content);
	return $content;
	
}

function morfeo_double_render($tag_string){
$contador=rand(9, 9999999);
	$site_url = get_option( 'siteurl' );
global $wpdb; 	
$table_name = $wpdb->prefix . "morfeo_double";	
	...
	if($auxivideo!="") {
		...
		if(strstr($auxivideo, "http")) {
			if(strpos($auxivideo, "youtube")>0) {
				$auxivideo=getYTiddouble($auxivideo);
				$auxtipo=2;
...
$texto.='&cantidad='.$cont1;
  ...
  <param name="movie" value="'.$site_url.'/wp-content/plugins/morfeo-double-video-gallery/morfeo_double.swf" />
  ...
  <param name="expressinstall" value="'.$site_url.'/wp-content/plugins/morfeo-double-video-gallery/Scripts/expressInstall.swf" />
  <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
  <!--[if !IE]>-->
  <object type="application/x-shockwave-flash" data="'.$site_url.'/wp-content/plugins/morfeo-double-video-gallery/morfeo_double.swf" width="'.$width.'" height="'.$height.'">
    <!--<![endif]-->
    <param name="quality" value="high" />
    <param name="wmode" value="transparent" />
    	<param name="flashvars" value="'.$texto.'" />
...
function morfeo_double_instala(){
	global $wpdb; 
	$table_name= $wpdb->prefix . "morfeo_double";
...
function widget_morfeo_double($args) {

 
  
    extract($args);
	
	  $options = get_option("widget_morfeo_double");
  if (!is_array( $options ))
{
$options = array(
      'title' => 'Morfeo double',
  ...
  echo $after_title;
  $aaux[1]=$options['id'];
 echo morfeo_double_render($aaux);
  echo $after_widget;
...
function morfeo_double_control()
{
  $options = get_option("widget_morfeo_double");
  if (!is_array( $options ))
{
$options = array(
  ...
  if ($_POST['morfeo-Submit'])
  {
    ...
    update_option("widget_morfeo_double", $options);
  }
...
if(empty($myrows)) {
	...
	echo '
	<p>First create a new gallery of videos, from the administration of double MORFEO plugin.</p>
	';
}
...
function morfeo_double_init(){
	register_sidebar_widget(__('Morfeo double'), 'widget_morfeo_double');
	register_widget_control(   'Morfeo double', 'morfeo_double_control', 300, 300 );
}

function morfeo_double_add_menu(){	
	if (function_exists('add_options_page')) {
		
		add_menu_page('morfeo_double', 'Morfeo double', 8, basename(__FILE__), 'morfeo_double_panel');
	}
}
if (function_exists('add_action')) {
	add_action('admin_menu', 'morfeo_double_add_menu'); 
}
add_action('wp_head', 'morfeo_double_head');
add_filter('the_content', 'morfeo_double');
add_action('activate_morfeo_double/morfeo_double.php','morfeo_double_instala');
add_action('deactivate_morfeo_double/morfeo_double.php', 'morfeo_double_desinstala');
add_action("plugins_loaded", "morfeo_double_init");
?>